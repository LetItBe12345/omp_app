...
- [x] 从用户 Login Shell 读取完整环境，覆盖从图形入口启动时缺失的 PATH 和普通变量。
- [x] Shell 探测超时或失败时退回 Electron 启动环境，并记录不含变量值的诊断。
- [x] 环境来源和网络规则分开解析。
- [x] 界面可以查看脱敏后的最终环境诊断。

### 环境检测

- [x] 检测 `omp`、`git`、`node` 和 `python`。
- [x] 显示 PATH。
- [x] 显示当前 Shell 和工作目录。
- [x] 诊断和复制结果只输出 Shell、PATH、Workspace、工具路径与版本、网络模式和脱敏代理来源。
- [x] 环境异常给出具体错误和重新检测入口。

### Runtime Network Profile

- [x] 在输入框顶部增加与模型、推理强度和权限同级的代理控件。
- [x] 工具栏文案使用“代理：关闭 / 自动 / 手动”。
- [x] 点击手动后，在同一 Popover 中进入二级配置面板。
- [x] 手动面板只接受本地 HTTP 代理端口，并生成 `http://127.0.0.1:<端口>`。
- [x] 端口不提供默认值、不扫描，取值范围为 `1–65535`。
- [x] 自动模式只重新探测 Login Shell；未检测到代理时不注入并正常启动。
- [x] 自动模式只检测到 SOCKS 代理时不注入，并提示改用本地 HTTP 入站。
- [x] 自动模式允许临时继承带认证的 HTTP 代理，但不保存、不显示凭据。
- [x] 手动模式生成 `PI_PROXY` 以及大小写的 HTTP_PROXY、HTTPS_PROXY、ALL_PROXY 和 NO_PROXY。
- [x] 代理模式始终绕过 `localhost`、`127.0.0.1` 和 `::1`，界面不暴露 Bypass 配置。
- [x] 不注入模式从最终 `env` 中显式移除 OMP 专用变量和所有大小写代理变量。
- [x] 工具栏按钮只显示当前模式；一级菜单展示本次解析来源、结果和错误。
- [x] 一级菜单底部提供只读环境诊断入口。
- [x] 全局保存网络模式和手动端口，切换 Workspace 后继续使用。
- [x] 离开手动模式后保留上次端口；关闭 Popover 时丢弃未应用草稿。
- [x] VPN 状态变化后由用户主动重新检测，不做后台轮询。
- [x] Agent 运行、存在 Follow-up 或 Provider 登录期间禁用代理控件。
- [x] Runtime 失败或停止时仍可修改代理，用于恢复。
- [x] 没有 Workspace 时允许保存配置，下次 Runtime 启动时应用。
- [x] 用户点击“应用并重启”后保存草稿、重启 OMP Runtime 并恢复当前 Session。
- [x] 应用后 Runtime 启动失败时保留新配置，不自动回退到直连。
